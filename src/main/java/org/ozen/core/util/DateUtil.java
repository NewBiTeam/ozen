...
public class DateUtil {
	...
	public static String getYYYYMMDDHHMMSSDate(Date date) {
		...
		min = StringUtils.leftPad(min, 2, "0");
	...
	public static String calendarAddToString(Date date, int field, int amount) {
		...
		c.setTime(date);
		c.add(field, amount);
		return DateUtil.dateToDateString(c.getTime(), "yyyy-MM-dd HH:mm");
	}
	 ...
	 * @return int 相差秒数
	 */
	public static int getDiffDateSecond(Date startTime, Date endTime) {
		long differ = 0;
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		try {
			startTime = sdf.parse(sdf.format(startTime));
			endTime = sdf.parse(sdf.format(endTime));
			differ = endTime.getTime() - startTime.getTime();
		} catch (ParseException e) {
			log.error(e.getMessage(), e);
		}
		return (int) (differ / 1000);
