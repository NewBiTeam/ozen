...
public class StringUtil extends StringUtils{
    ...
    public static String toCamelName(String s)
    {
        if(s != null && isNotBlank(s))
        {
            s = s.toLowerCase();
            Pattern pattern = Pattern.compile("(_[a-z])", 2);
            Matcher matcher = pattern.matcher(s);
            StringBuffer stringbuffer = new StringBuffer();
            for(; matcher.find(); matcher.appendReplacement(stringbuffer, matcher.group().toUpperCase()));
            matcher.appendTail(stringbuffer);
            s = stringbuffer.toString().replace("_", "");
        }
        return s;
    }
    /**
